...
import com.box.app.provision.state.StartupState;
import com.box.app.provision.state.StateMachine;
import com.box.app.provision.utils.IKeyEvent;
import com.box.app.provision.utils.PageIntercepHelper;
import com.box.app.provision.utils.ProvisionStateHolder;

import com.box.app.provision.base.OobeUtils;
import com.box.app.provision.base.ProvisionBaseActivity;
...
public class DefaultActivity extends ProvisionBaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // 系统返回键：首屏（StartupState）时拦截，不允许退出引导
        getOnBackPressedDispatcher().addCallback(this, new OnBackPressedCallback(true) {
            @Override
            public void handleOnBackPressed() {
                if (mStateMachine != null && mStateMachine.getCurrentState() instanceof StartupState) {
                    // 首屏不响应返回键，防止退出引导后黑屏
                    return;
                }
                // 非首屏委托给 StateMachine 处理后退
    ...
    public void finishSetup() {
        finish();
        // 抑制 Activity 过渡，入场动画由 Compose 侧 AppScaffold 接管
